...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
    ...
    {"allowjoin", G_admin_allowjoin, "denyjoin",
    ...
    /* {"ambush_donate", G_admin_ambush_donate, "ambush_donate",
      "donate to upgrade human buildings",
      ""
    }, */

    {"ambush_stage", G_admin_ambush_stage, "ambush_stage",
    ...
    {"denyjoin", G_admin_denyjoin, "denyjoin",
      "disable joining of teams for a player",
      "[^3name|slot#^7]"
    },

    {"drawnodes", G_drawnodes, "drawnodes",
      "show or hide nodes",
      ""
    },
    ...
    {"stage", G_admin_stage, "stage",
      "change the stage for a team. Should only be used for training",
      "[^3#^7] [^3h^7|^3a^7|^3b^7]"
    },
...
qboolean G_admin_botcmd( gentity_t *ent, int skiparg ) {
	...
	minargc = 3 + skiparg;
	if( G_SayArgc() < minargc )	{
		ADMP( "^3!botcmd: ^7usage: !botcmd [botname] [regular|idle|standground|followattack|followidle]\n" );
		return qfalse;
	}
...
qboolean G_admin_kick( gentity_t *ent, int skiparg )
{
  ...
  {     
    ADMP( "Use !bot del name to kick a bot.\n" );
    return qfalse;
  }
...
qboolean G_admin_leave( gentity_t *ent, int skiparg )
{
  ...
  trap_DropClient( pids[ 0 ], va( "was forced to leave%s^7, reason: %s%s",
    ...
    if ( i == g_currentMapRotation.integer )
    {
      ...
      for( j = 0; j < mapRotations.rotations[ i ].numMaps; j++ )
      {
        ...
        ADMBP( va( "^%i%3i %-20s ^%i%s\n       \\__ layouts: %s\n",
            ...
            statusColor,
            status,
            mapRotations.rotations[ i ].maps[ j ].layouts ) );
      }
...
qboolean G_admin_showbans( gentity_t *ent, int skiparg )
{
    ...
    && show_count < MAX_ADMIN_SHOWBANS; i++ )
  {
    ...
    ADMBP( va( "%4i %s^7 %-15s ^7 %-8s %s^7 %-10s\n     \\__ %s\n",
             ( i + 1 ),
             n1,
             g_admin_bans[ i ]->ip,
             date,
...
qboolean G_admin_lock( gentity_t *ent, int skiparg )
{
  ...
  if( teamName[ 0 ] == 'a' || teamName[ 0 ] == 'A' )
    ...
    team = PTE_HUMANS;
  else if( teamName[ 0 ] == 'b' || teamName[ 0 ] == 'B' )
    team = PTE_NONE;
  else
  {
  ...
  if( team == PTE_ALIENS )
  {
    ...
    {
      ADMP( "^3!lock: ^7both teams are already locked\n" );
      return qfalse;
    }
    else
    {
  ...
  {
    AP( va( "print \"^3!lock: ^7both teams were locked by %s\n\"",
      ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }
...
qboolean G_admin_unlock( gentity_t *ent, int skiparg )
{
  ...
  if( team == PTE_ALIENS )
  {
    ...
    if( !level.humanTeamLocked && !level.alienTeamLocked )
    {
      ADMP( "^3!unlock: ^7neither team is currently locked\n" );
      return qfalse;
    }
    else
    {
  ...
  {
    AP( va( "print \"^3!unlock: ^7both teams were unlocked by %s\n\"",
      ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }
...
qboolean G_admin_putmespec( gentity_t *ent, int skiparg )
{
  ...
  if( ent->client->pers.teamSelection == PTE_HUMANS && ent->client->grabExpiryTime > level.time )
  {
    trap_SendServerCommand( ent-g_entities,
        va( "print \"You cannot change teams while a basilisk is holding you\n\"" ) );
    return qfalse;
  }
...
qboolean G_password(gentity_t *ent, int skiparg )
{
  ...
  minargc = 2 + skiparg;
  if( !g_pathpassword.string[0] ){ADMP( "^3!password: Not set (g_pathpassword)\n" );return qfalse;}
  if( G_SayArgc() < minargc )
  {
...
qboolean G_admin_ambush_stage(gentity_t *ent,int skiparg)
{
	if(g_ambush.integer <= 0)
	{
		ADMP("^1Ambush Mode is off\n");
		return qfalse;
	}
...
/* qboolean G_admin_ambush_donate(gentity_t *ent,int skiparg)
{
	...
	if(!ent->client || ent->client->pers.teamSelection != PTE_HUMANS)
	{
		ADMP("^1You are not a human\n");
		return qfalse;
	}
	minargc = 2 + skiparg;
	if( G_SayArgc() < minargc )
	{
		ADMP(va("^3!donate: ^2Current Donations: ^5%d\n",(int)level.donations));
		ADMP(va("^2Next Upgrade level %d: %d percent\n",g_ambush_upgrade.integer + 1,(int)(((float)level.donations / ((float)g_ambush_upgrade_cost.integer * ((float)level.averageNumHumanClients + 1.0f))) * 100.0f)));
		ADMP( "^7usage: !donate [amount]\n" );
		return qfalse;
	}
	G_SayArgv( 1 + skiparg, donation, sizeof(donation) );
	amount = atoi(donation);
	if(amount > ent->client->pers.credit)
	{
		amount = ent->client->pers.credit;
	}
	if(amount < 500)
	{
		ADMP("^3!donate: Minimum is 500 to prevent spam.\n");
		return qfalse;
	}
	level.donations += amount;
	G_AddCreditToClient( ent->client, -amount, qfalse );
	//ent->client->pers.credit -= amount;
	AP( va( "print \"^3!donate: ^7%s^7 donated %d credits\n\"", ent->client->pers.netname,amount));
	if(((float)level.donations / ((float)g_ambush_upgrade_cost.integer * ((float)level.averageNumHumanClients + 1.0f))) >= 1)
	{
		AP( va( "print \"^2!Buildings have been upgraded to level %d!\n\"", g_ambush_upgrade.integer + 1));
		level.donations -= ((float)g_ambush_upgrade_cost.integer * ((float)level.averageNumHumanClients + 1.0f));
...
qboolean G_admin_denyjoin( gentity_t *ent, int skiparg )
{
  ...
  if( G_SayArgc() < minargc )
  {
    ADMP( "^3!denyjoin: ^7usage: !denyjoin [name|slot#]\n" );
    return qfalse;
  }
  ...
  {
    ADMP( "^3!denyjoin: ^7player already forcespeced\n" );
    return qfalse;
  }
  ...
  AP( va( "print \"^3!denyjoin: ^7%s^7 has disallowed joining of teams for ^7%s\n\"",
...
qboolean G_admin_allowjoin( gentity_t *ent, int skiparg )
{
  int pids[ MAX_CLIENTS ];
  char name[ MAX_NAME_LENGTH ], err[ MAX_STRING_CHARS ];
  int minargc;
  ...
  if( G_SayArgc() < minargc )
  {
    ADMP( "^3!allowjoin: ^7usage: !allowjoin [name|slot#]\n" );
    return qfalse;
  }
  ...
  {
    ADMP( "^3!allowjoin: ^7player is not forcespeced\n" );
    return qfalse;
  }
  ...
  AP( va( "print \"^3!allowjoin: ^7%s^7 has allowed joining of teams for ^7%s\n\"",
      ( ent ) ? ent->client->pers.netname : "console", vic->client->pers.netname ) );
  return qtrue;
}
...
qboolean G_admin_stage( gentity_t *ent, int skiparg )
{
  ...
  G_SayArgv( 2 + skiparg, teamName, sizeof( teamName ) );
  ...
  // Handle invalid stage input
  if( ( team == PTE_HUMANS || team == PTE_NONE ) &&
      ( lvl > atoi( DEFAULT_HUMAN_MAX_STAGE ) ||
      lvl < atoi( DEFAULT_HUMAN_MIN_STAGE ) ) )
  {
    ADMP( va( "^3!%s: ^7%i is an invalid human stage\n", cmd, lvl+1 ) );
    return qfalse;
  }
  else if( ( team == PTE_ALIENS || team == PTE_NONE ) &&
      ( lvl > atoi( DEFAULT_ALIEN_MAX_STAGE ) ||
      lvl < atoi( DEFAULT_ALIEN_MIN_STAGE ) ) )
  {
    ADMP( va( "^3!%s: ^7%i is an invalid alien stage\n", cmd, lvl+1 ) );
    return qfalse;
  }
  ...
  if( team == PTE_HUMANS )
  {
    ...
    trap_Cvar_Set( "g_humanMaxStage", lvl_chr );
    AP( va( "print \"^3!%s: ^7the human team has been given stage ^1%i^7 by %s\n\"",
        cmd, lvl+1,
        ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }  
  else if( team == PTE_ALIENS )
  {
    ...
    trap_Cvar_Set( "g_alienMaxStage", lvl_chr );
    AP( va( "print \"^3!%s: ^7the alien team has been given stage ^1%i^7 by %s\n\"",
        cmd, lvl+1, 
        ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }
  else
  {
    ...
    trap_Cvar_Set( "g_alienStage", lvl_chr );
    trap_Cvar_Set( "g_alienMaxStage", lvl_chr );
    AP( va( "print \"^3!%s: ^7both teams have been given stage ^1%i^7 by %s\n\"",
        cmd, lvl+1,
        ( ent ) ? G_admin_adminPrintName( ent ) : "console" ) );
  }
...
qboolean G_admin_bp( gentity_t *ent, int skiparg )
{
  char bp_chr[ MAX_STRING_CHARS ];
  int minargc;
  int bp;
  char command[ MAX_ADMIN_CMD_LEN ], *cmd;
  char teamName[2] = {""};
  ...
  G_SayArgv( 1 + skiparg, bp_chr, sizeof( bp_chr ) );
  bp = atoi(bp_chr);

  if( bp < 0 )
  {
    ADMP( va( "^3!%s: ^7you cannot set build points lower than zero\n", cmd ) );
    return qfalse;
  }
  ...
  if( team == PTE_HUMANS )
  {
    trap_Cvar_Set( "g_humanBuildPoints", bp_chr );
    AP( va( "print \"^3!%s: ^7%s^7 has set the human build points to ^1%i\n\"",
        cmd,
        ( ent ) ? G_admin_adminPrintName( ent ) : "console",
        bp )
    );
  ...
  {
    trap_Cvar_Set( "g_alienBuildPoints", bp_chr );
    AP( va( "print \"^3!%s: ^7%s^7 has set the alien build points to ^1%i\n\"",
        cmd,
        ( ent ) ? G_admin_adminPrintName( ent ) : "console",
        bp )
    );
  ...
  else
  {
    trap_Cvar_Set( "g_humanBuildPoints", bp_chr );
    trap_Cvar_Set( "g_alienBuildPoints", bp_chr );
    AP( va( "print \"^3!%s: ^7%s^7 has set both teams' build points to ^1%i\n\"",
        cmd,
        ( ent ) ? G_admin_adminPrintName( ent ) : "console",
        bp )
    );
