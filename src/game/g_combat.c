...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
{
  ...
  if( attacker && attacker->client )
  {
    ...
    if( ( g_KillerHP.integer || ( g_devmapKillerHP.integer && g_cheats.integer ) ) &&
        attacker != self )
    {
      trap_SendServerCommand( self-g_entities,
          va( "print \"Your killer, %s^7, had %3i HP.\n\"",
              killerName,
              ( attacker->health < 0 ) ? 0 : attacker->health ) );
    }
    ...
    else
    {
      ...
      {
         //ROTAX
     	  if (g_ambush.integer > 0 && ROTACAK_ambush_rebuild_time_temp < level.time && !tk)//V rebuild time nepocitam killy
      	  {
 		ROTACAK_ambush_kills++;
 		if(g_ambush_stage.integer == AMBUSH_MAX_WAVE &&
                    level.numHumanClients * g_ambush_killstonextstage.integer < ROTACAK_ambush_kills )
 		{
 			trap_Cvar_Set( "g_ambush_stage", va( "%d", g_ambush_stage.integer + 1 ) );
 			for( i = 1, e = g_entities + i; i < level.num_entities; i++, e++ )
 			{
 				if( e->s.modelindex == BA_A_SPAWN )
 				{
 					e->health = 0;
 					G_Damage( e, NULL, NULL, NULL, NULL, 10000, 0, MOD_TARGET_LASER );
 				}
 			}
 			trap_SendServerCommand( -1, va( "cp \"^1 ====> ^7WELL DONE ^1- ^7NO EGGS ^1<====  ^7\"" ) );
 		}
 		if(level.numHumanClients * g_ambush_killstonextstage.integer < ROTACAK_ambush_kills){ambush_next_stage();}
         }
  ...
  if( totalDamage > 0.0f )
  {
    if( self->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS )
    {
      ...
      // long double classValue = self->client->pers.maxHealth;
  ...
  self->client->respawnTime = level.time + 500;//1700;
         ...
         vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
  ...
  //if( targ->biteam == BIT_ALIENS && g_ambush_stage.integer < 36 && g_ambush.integer > 0 )
  //	return;

  
  if ( g_abuild_dmg.integer <= 0 &&
	 !targ->client && targ->biteam == BIT_ALIENS && mod != MOD_TARGET_LASER)
    return;
  else if ( g_hbuild_dmg.integer <= 0 &&
  ...
  if( knockback && targ->client )
  {
    ...
    if(targ == attacker)
    {
	VectorScale( dir, g_selfknockback.value * (float)knockback / mass, kvel );
	// damage = 1;//damage / 2;
    }
    else
    {
...
void ambush_next_stage()
{
  ...
  stage = g_ambush_stage.integer + 1;
  trap_Cvar_Set( "g_ambush_stage", va( "%d", g_ambush_stage.integer + 1 ) );
  ...
  trap_SendServerCommand( -1, va( "cp \"^1--> ^7Prepare for Wave %i^1 <--^7\n\"", stage ) );
  trap_SendServerCommand( -1, va( "print \"^1--> ^7Prepare for Wave %i^1 <--^7\n\"", stage ) );
  ...
  {
	case 3:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Granger Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Granger Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 6:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Adv Granger Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Adv Granger Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 9:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Dretch Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Dretch Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 12:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Basilisk Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Basilisk Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 15:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Adv Basilisk Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Adv Basilisk Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 18:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Marauder Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Marauder Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 21:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Adv Marauder Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Adv Marauder Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 24:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Dragoon Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Dragoon Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 27:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Adv Dragoon Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Adv Dragoon Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	case 30:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for Tyrant Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\"", stage, string ) );
		trap_SendServerCommand( -1, va( "print \"^1--> Prepare for Tyrant Boss (^2Stage %i^1) <--\n^1(attack in %s)^7\n\"", stage, string ) );
		level.boss = qtrue;
		break;
	default:
		trap_SendServerCommand( -1, va( "cp \"^1--> Prepare for next wave (^2Stage %i^1) <--\n^1(attack at %s)^7\"", stage ) );
