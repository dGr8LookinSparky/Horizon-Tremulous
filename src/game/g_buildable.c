...
// OVERMIND DEFINES WERE MOVED TO G_LOCAL.H
...
//TRAPPER DEFINE MOVED TO G_LOCAL.H
...
// DCC ATTACK PERIOD MOVED TO G_LOCAL.H
...
void G_BuildableThink( gentity_t *ent, int msec )
{
  ...
  */
  ...
  {
    bHealth += (g_ambush_upgrade.integer * bHealth) / 2;
  } */
  ...
  if(ent->biteam == BIT_ALIENS || g_buildMode.integer != 3 )
  {
    ent->time1000 += msec;
...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin )
{
  ...
  VectorCopy( tr1.plane.normal, normal );//Mark this out?
...
static gentity_t *G_Build( gentity_t *builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
  ...
  // build instantly in cheat mode or g_buildMode 2
  if( builder->client && ( g_cheats.integer || g_buildMode.integer == 2) )
  {
  ...
  bHealth = BG_FindHealthForBuildable( buildable );
  /* if(built->biteam == BIT_HUMANS)
  {
    bHealth += (g_ambush_upgrade.integer * bHealth) / 2;
  } */
  built->s.generic1 = (int)( ( (float)built->health /
        (float)bHealth ) * B_HEALTH_MASK );
...
qboolean G_BuildIfValid( gentity_t *ent, buildable_t buildable )
{
  ...
  switch( G_CanBuild( ent, buildable, dist, origin ) )
  {
    ...
    default://G_Build( ent, buildable, origin, ent->s.apos.trBase );
      break;
  }
