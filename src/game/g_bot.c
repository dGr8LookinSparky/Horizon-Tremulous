...
void G_BotAdd( char *name, int team, int skill, int ignore ) {
	...
	if(skill > BOT_MAX_SKILL) skill = BOT_MAX_SKILL;
	if(skill < BOT_MIN_SKILL) skill = BOT_MIN_SKILL;
...
void G_BotCmd( gentity_t *master, int clientNum, char *command ) {
  ...
  {
    trap_Printf( va("You can't change aliens behavior during ambush mode\n") );
    return;
  }*/
...
{
	if(!( self->r.svFlags & SVF_BOT ) || self->client->ps.stats[ STAT_PCLASS ] == PCL_NONE)
	{
		return;
	}
	...
	if(!self->botEnemy && !self->botFriend && self->state == TARGETPATH)
	{
			...
			VectorLength( self->client->ps.velocity ) < 50.0f) && level.time - self->enemytime > 1000)
		{
			...
			if(level.time - self->jumptime > 3000 &&
				(( self->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && self->client->ps.stats[ STAT_STAMINA ] > 0 ) ||
...
void Bot_Evolve( gentity_t *self )
{
	...
	{
		classfound = qtrue;
		if(self->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_BUILDER0)
		{
			int randclass = 0;
			int randmax = 0;
			srand( trap_Milliseconds( ) );
        ...
        else if( g_bot_evolve.integer <= 0 ) return; // ambush and evolving are off
...
void G_BotThink( gentity_t *self )
{
	...
	if(self->client->ps.stats[ STAT_PTEAM ] == PTE_ALIENS &&
		...
		self->boss == qfalse && 
		(ROTACAK_ambush_rebuild_time_temp < level.time && ((level.time - level.startTime) > (g_ambush_sec_to_start.integer * 1000))) )
	{Bot_Evolve(self);}
	if(self->client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS)
	{
...
{
	if( g_bot.integer <= 0 || self->client->ps.pm_flags & PMF_QUEUED || self->client->ps.stats[ STAT_PCLASS ] != PCL_NONE)
	{
...
int botFindClosestEnemy( gentity_t *self, qboolean includeTeam ) {
	...
	target->s.modelindex == BA_A_HOVEL)) || (!target->client && self != target && self->client->ps.stats[ STAT_PCLASS ] == PCL_ALIEN_LEVEL0 && (target->s.modelindex == BA_H_MGTURRET || target->s.modelindex == BA_H_TESLAGEN)))
      {
